...
def test_check_header():
    errors = util.check_header('1', 0)
    assert 'Number Header' in errors[0]

    errors = util.check_header('n.a.', 0)
    assert 'NA Header' in errors[0]

    errors = util.check_header('\\', 0)
    assert 'Illegal Header' in errors[0]

    errors = util.check_header('social_security', 0)
    assert 'PHI Header' in errors[0]

    errors = util.check_header(' France', 0)
    assert 'Illegal Header' in errors[0]
...
def test_check_column():
    ...
    errors, warnings = util.check_column(test_df['Test']['StudyName'], 0)
    assert 'Multiple studies in one' in errors[0]

    errors, warnings = util.check_column(test_df['Test']['Non-standard1'], 0)
    assert 'Non standard NA format' in errors[0]

    errors, warnings = util.check_column(test_df['Test']['Non-standard2'], 0)
    assert 'Non standard NA format' in errors[0]

    #  errors, warnings = util.check_column(test_df['Test']['MixedTypes'], 0)
    #  assert 'Mixed Type' in errors[0]

    errors, warnings = util.check_column(test_df['Test']['EmptyCell1'], 0)
    #  assert 'Mixed Type' in errors[0]
    assert 'Empty cell value' in errors[1]

    errors, warnings = util.check_column(test_df['Test']['EmptyCell2'], 0)
    assert 'Empty cell value' in errors[0]

    errors, warnings = util.check_column(test_df['Test']['Categorical'], 0)
    assert 'Categorical Data' in warnings[0]

    errors, warnings = util.check_column(test_df['Test']['Whitespace'], 0)
    assert 'Whitespace' in errors[0]

    errors, warnings = util.check_column(test_df['Test']['BadDate'], 0)
    assert 'Date Error' in errors[0]

    errors, warnings = util.check_column(test_df['Test']['GoodDate'], 0)
    assert len(errors) == 0 and len(warnings) == 0

    errors, warnings = util.check_column(test_df['Test']['GoodString'], 0)
    assert len(errors) == 0 and len(warnings) == 0

    errors, warnings = util.check_column(test_df['Test']['GoodNumber'], 0)
    assert len(errors) == 0 and len(warnings) == 0
...
def test_check_duplicates():
    test_df = pd.read_csv(fig.TEST_METADATA_VALID, header=[0, 1], sep='\t', na_filter=False)
    errors = util.check_duplicates(test_df['Test']['GoodNumber'], 0)
    assert len(errors) == 0

    errors = util.check_duplicates(test_df['Test']['GoodString'], 0)
    assert 'Duplicate Value' in errors[0]


def test_check_lengths():
    test_df = pd.read_csv(fig.TEST_METADATA_VALID, header=[0, 1], sep='\t', na_filter=False)
    errors = util.check_lengths(test_df['Test']['GoodBarcodes'], 0)
    assert len(errors) == 0

    errors = util.check_lengths(test_df['Test']['BadBarcodes'], 0)
    assert 'different length from other' in errors[0]


def test_check_barcode_chars():
    test_df = pd.read_csv(fig.TEST_METADATA_VALID, header=[0, 1], sep='\t', na_filter=False)
    errors = util.check_barcode_chars(test_df['Test']['GoodBarcodes'], 0)
    assert len(errors) == 0

    errors = util.check_barcode_chars(test_df['Test']['BadBarcodes'], 0)
...
def test_check_duplicate_cols():
    ...
    dups = util.check_duplicate_cols(headers)
    assert len(dups) > 0


def test_check_dates():
    test_df = pd.read_csv(fig.TEST_METADATA_VALID, header=[0, 1], sep='\t', na_filter=False)
    errors = util.check_dates(test_df['BadDates'])
    assert 'earlier than start date' in errors[0]

    errors = util.check_dates(test_df['GoodDates'])
    assert len(errors) == 0


def test_validate_mapping_files():
    with open(fig.TEST_METADATA) as f:
        errors, warnings, study_name, subjects = util.validate_mapping_file(f)
    assert not errors
    assert not warnings
...
def test_check_ICD_code():
    ...
    errors = util.check_ICD_codes(valid_codes, 0)
    assert not errors
    errors = util.check_ICD_codes(invalid_codes, 0)
    assert len(errors) == 5
