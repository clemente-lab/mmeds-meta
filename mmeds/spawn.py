...
from mmeds.database.id_generators import AliquotUploader
...
class Watcher(BaseManager):
    ...
    def handle_upload(self, process):
        ...
        if self.current_upload is None:
            ...
            if 'aliquot' in process[0]:
                (ptype, owner, access_code, aliquot_table) = process
                p = AliquotUploader(owner, access_code, aliquot_table, self.testing)
            else:
