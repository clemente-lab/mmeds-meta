...
from mmeds.util import write_metadata
...
def write_error_files():
    ...
    write_metadata(issue_parsing, '{}/validate_error_issue_parsing.tsv'.format(file_path))
    ...
    write_metadata(wrong_type, '{}/validate_error_wrong_type.tsv'.format(file_path))
    ...
    write_metadata(illegal_header, '{}/validate_error_illegal_header.tsv'.format(file_path))
    ...
    col.rename(('AdditionalMetaData', 'IllegalColumn_'), inplace=True)
    illegal_header = illegal_header.join(col, how='outer')
    write_metadata(illegal_header, '{}/validate_error_illegal_header.2.tsv'.format(file_path))
    ...
    write_metadata(column_name, '{}/validate_error_column_name.tsv'.format(file_path))
    ...
    write_metadata(illegal_column, '{}/validate_error_illegal_column.tsv'.format(file_path))
    ...
    write_metadata(illegal_table, '{}/validate_error_illegal_table.tsv'.format(file_path))

    # Missing a column
    missing_column = df.copy(deep=True)
    missing_column.drop(('Subjects', 'HostSubjectId'), axis=1, inplace=True)
    write_metadata(missing_column, '{}/validate_error_missing_column.tsv'.format(file_path))

    # Missing a table
    missing_table = df.copy(deep=True)
    missing_table.drop('Subjects', axis=1, inplace=True)
    write_metadata(missing_table, '{}/validate_error_missing_table.tsv'.format(file_path))
    ...
    write_metadata(empty_cell, '{}/validate_error_empty_cell.tsv'.format(file_path))
    ...
    write_metadata(missing_type, '{}/validate_error_missing_type.tsv'.format(file_path))
    ...
    write_metadata(invalid_type, '{}/validate_error_invalid_type.tsv'.format(file_path))

    # Whitespace
    whitespace = df.copy(deep=True)
    whitespace.iloc[10][('BodySite', 'SpecimenBodySite')] = \
        ' ' + whitespace.iloc[10][('BodySite', 'SpecimenBodySite')] + ' '
    write_metadata(whitespace, '{}/validate_error_whitespace.tsv'.format(file_path))
    ...
    write_metadata(missing_required_value, '{}/validate_error_missing_required_value.tsv'.format(file_path))
    ...
    write_metadata(non_standard_na, '{}/validate_error_non_standard_na.tsv'.format(file_path))
    ...
    write_metadata(number_header, '{}/validate_error_number_header.tsv'.format(file_path))
    ...
    write_metadata(na_header, '{}/validate_error_na_header.tsv'.format(file_path))
    ...
    write_metadata(phi_header, '{}/validate_error_phi_header.tsv'.format(file_path))
    ...
    duplicate_column = duplicate_column.join(col, how='outer')
    write_metadata(duplicate_column, '{}/validate_error_duplicate_column.tsv'.format(file_path))
    ...
    write_metadata(duplicate_value, '{}/validate_error_duplicate_value.tsv'.format(file_path))
    ...
    write_metadata(different_length, '{}/validate_error_different_length.tsv'.format(file_path))
    ...
    write_metadata(invalid_barcodesequence, '{}/validate_error_invalid_barcodesequence.tsv'.format(file_path))

    # invalid_icd_code
    invalid_icd_code = df.copy(deep=True)
    invalid_icd_code.iloc[10]['ICDCode']['ICDCode'] = 'NotACode'
    write_metadata(invalid_icd_code, '{}/validate_error_invalid_icd_code.tsv'.format(file_path))
    ...
    write_metadata(multiple_studies, '{}/validate_error_multiple_studies.tsv'.format(file_path))
    ...
    write_metadata(invalid_date_range, '{}/validate_error_invalid_date_range.tsv'.format(file_path))

    # future_date
    future_date = df.copy(deep=True)
    future_date.iloc[10]['ResultsProtocol']['ResultsDatePerformed'] = '2222-2-1'
    write_metadata(future_date, '{}/validate_error_future_date.tsv'.format(file_path))

    # Length error
    cell_length = df.copy(deep=True)
    cell_length.iloc[10]['ResultsProtocol']['ResultsProcessor'] = 'asdf' * 40
    write_metadata(cell_length, '{}/validate_error_cell_length.tsv'.format(file_path))
...
def write_warning_files():
    ...
    write_metadata(stddev_warning, '{}/validate_warning_stddev_warning.tsv'.format(file_path))

    # categorical_data
    categorical_data = df.copy(deep=True)
    categorical_data.iloc[10]['ResultsProtocols']['ResultsMethod'] = 'Protocol90'
    write_metadata(categorical_data, '{}/validate_warning_categorical_data.tsv'.format(file_path))
