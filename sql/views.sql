...
DROP VIEW IF EXISTS `mmeds`.`protected_CollectionSite`;
CREATE
SQL SECURITY DEFINER
VIEW protected_CollectionSite AS
SELECT cc.* FROM `mmeds`.`CollectionSite` cc WHERE owner_check(cc.user_id)
WITH CHECK OPTION;

GRANT SELECT ON TABLE `mmeds`.`protected_CollectionSite` TO 'mmeds_user'@'%';
