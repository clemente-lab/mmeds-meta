...
CREATE
SQL SECURITY DEFINER
VIEW `mmeds_data1`.`protected_Location` AS
SELECT cc.* FROM `mmeds_data1`.`Location` cc WHERE `mmeds_data1`.`owner_check`(cc.user_id)
WITH CHECK OPTION;

GRANT SELECT ON TABLE `mmeds_data1`.`protected_Location` TO 'mmedsusers'@'%';
